...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <div>
        <h4 class="mb-0"><i class="fas fa-user-nurse text-primary me-2"></i> Jasa Medis</h4>
...
function buildTable(columns, data, title, summary) {
    ...
    dtJasaMedis = $('#tblJasaMedis').DataTable({
        ...
        buttons: [
            {
                extend: 'excelHtml5',
                title: 'Jasa Medis - ' + (title || 'Detail'),
                filename: function() {
                    return 'Jasa_Medis_' + activeTab + '_' + $('#tglAwal').val().replace(/-/g, '') + '_sd_' + $('#tglAkhir').val().replace(/-/g, '');
                },
                className: 'd-none',
                exportOptions: {
                    columns: ':visible',
