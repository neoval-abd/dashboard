...
// Logika D: Hitung semua baris mapping SK (Termasuk Pindah Kamar)
$sql_transaksi = "
    SELECT 
        kamar.kd_bangsal,
...
$final_data = [];
foreach ($bangsal_data as $kd_bangsal => $row) {
    $bed = $row['bed'];
    $hp = calculateSkHariPerawatan($koneksi, $tgl_awal, $tgl_akhir, $kd_bangsal);
    $d = $row['d'];
    $mati = $row['mati'];
    $mati_48 = $row['mati_48'];
